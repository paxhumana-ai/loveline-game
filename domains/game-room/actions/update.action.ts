...
export async function updateGameRoomSettings(
  ...
  success: boolean;
  data?: typeof gameRooms.$inferInsert;
  error?: string;
}> {
  try {
    // 입력값 검증
...
export async function startGame(gameRoomId: string): Promise<{
  success: boolean;
  data?: typeof gameRooms.$inferInsert;
  error?: string;
}> {
  try {
    ...
    const result = await db.rls(async (tx) => {
      ...
      const notReadyParticipants = participantList.filter(
        (p) => p.status !== "ready"
      );
...
export async function updateParticipantStatus(
  ...
  success: boolean;
  data?: typeof participants.$inferInsert;
  error?: string;
}> {
  try {
    const db = await createDrizzleSupabaseClient();
