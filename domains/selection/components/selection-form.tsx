...
interface SelectionFormProps {
  ...
  onSubmit: (data: SelectionInput) => Promise<{ success: boolean; error?: string }>;
  disabled?: boolean;
  existingSelection?: {
...
export function SelectionForm({
  ...
  const {
    ...
    defaultValues: {
      roundId,
      selectedParticipantId: existingSelection?.selectedParticipantId || undefined,
      message: existingSelection?.message || "",
      isPassed: existingSelection?.isPassed || false,
    },
  });
  ...
  const selectableParticipants = participants.filter(p => p.id !== currentParticipantId);
  ...
  const handleFormSubmit = async (data: SelectionInput) => {
          ...
          {(hasSelectedParticipant || isPassed) && (
            <div className="text-center p-4 bg-primary/5 rounded-lg">
              <p className="text-sm">
                {isPassed 
                  ? "이번 라운드를 패스합니다" 
                  : `${participants.find(p => p.id === watchedValues.selectedParticipantId)?.nickname}님을 선택했습니다`
                }
              </p>
